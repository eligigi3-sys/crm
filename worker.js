...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path.startsWith('/api/')) {
      try {
        let response;
        if (path.startsWith('/api/auth')) response = await handleAuth(request, env, path);
        else if (path.startsWith('/api/dashboard')) response = await handleDashboard(request, env, path);
        else if (path.startsWith('/api/leads')) response = await handleLeads(request, env, path);
        else if (path.startsWith('/api/google')) response = await handleGoogle(request, env, path);
        else if (path.startsWith('/api/contacts')) response = await handleContacts(request, env, path);
        else response = { error: 'Route not found' };
